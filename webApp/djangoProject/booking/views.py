...
from datetime import datetime, timedelta,date
from riskAssesment.models import Symptoms, Hygiene, Travel
...
def booking_view(request):
    ...
    if request.method == 'POST':
        ...
        queryset = Booking.objects.all().order_by('-booked_date').filter(booked_date__iexact=form['booked_date'].value())
            ...
            if user >= 1:
                ...
                if week >= 3:
                    ...
                    if occupied >= 30:
                        ...
                        if hygiene.status =="Low Risk" or travel.status =="Low Risk":
...
def admin_view(request):
    queryset = Booking.objects.all().order_by('booked_date')
    ...
    if request.method == 'POST':
        queryset = Booking.objects.all().order_by('-booking_date').filter(booked_date__icontains=form['booked_date'].value())
        context = {
            "queryset" : queryset,
            "form" : form,
        }
